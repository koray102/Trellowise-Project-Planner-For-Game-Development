...
const MOCK_TASKS: TaskItem[] = [
  { id: 't1', projectId: 'proj_1', title: 'Fix jumping physics bug', description: 'Player occasionally double jumps when hitting a slope.', assignedTo: '3', status: 'progress', sort_order: 0 },
  { id: 't2', projectId: 'proj_1', title: 'Design Level 2 layout', description: 'Focus on verticality and adding new enemy types.', assignedTo: '4', status: 'todo', sort_order: 0 },
  { id: 't3', projectId: 'proj_1', title: 'Create main character animations', description: 'Attack, Dash, and Idle loops.', assignedTo: '2', status: 'progress', sort_order: 1 },
  { id: 't4', projectId: 'proj_1', title: 'Implement audio manager', description: 'Add support for spatial 3D audio in Unity.', assignedTo: '1', status: 'done', sort_order: 0 },
  { id: 't5', projectId: 'proj_1', title: 'Refactor UI code (Technical Debt)', description: 'Move from old canvas system to UI Toolkit.', assignedTo: '1', status: 'debt', sort_order: 0 },
];
...
export const useTaskStore = create<TaskState>((set, get) => ({
  ...
  addTask: async (title, description, assignedTo, status) => {
    ...
    const newTask: TaskItem = { id, projectId, title, description, assignedTo, status, sort_order };

    // Optimistic: add immediately
    set((state) => ({ tasks: [...state.tasks, newTask] }));

    try {
      await insertTask({ id, projectId, title, description, assignedTo, status, sort_order });
